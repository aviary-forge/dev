...
let
  ips = [
    "10.67.192.214/32"
    "fc00:bbbb:bbbb:bb01::4:c0d5/128"
  ];
  nameservers = [
    "8.8.8.8"
    "8.8.4.4"
...
in
{
  options.dev.denbeigh.wireguard =
    with lib.options;
    with lib.types;
    {
  ...
  config = {
    ...
    networking =
      let
        ...
        vpn = with config.dev.denbeigh.wireguard; {
        ...
        routeUsers = unames: num: builtins.concatStringsSep "\n" (map (u: routeUser u num) unames);
        ...
        unRouteUsers = unames: num: builtins.concatStringsSep "\n" (map (u: unRouteUser u num) unames);

        cfg = config.dev.denbeigh.wireguard;
